...
const { Incident, User, Status, Priority, Comment, Actuacions} = require('../db');
...
router.get('/', async (req, res) => {
    try {
        ...
        res.render('Actuacions/list', { incidents });
    } catch (error) {
        console.error("Error al recuperar Actuacions:", error);
        res.status(500).send('Error al recuperar Actuacions');
    }
});
...
router.get('/new', async (req, res) => {
    try {
        ...
        res.render('Actuacions/new', { users, statuses, priorities });
    } catch (error) {
